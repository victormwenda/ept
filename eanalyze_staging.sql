...
--
-- Host: 127.0.0.1
-- Generation Time: Feb 03, 2021 at 08:34 AM
...
--
-- Stand-in structure for view `individual_final_report`
-- (See below for the actual view)
--
CREATE TABLE `individual_final_report` (
`shipment_id` int(11)
,`shipment_code` varchar(255)
,`shipment_date` date
,`mean_shipment_score` decimal(10,6)
);
...
--
-- Structure for view `individual_final_report`
--
DROP TABLE IF EXISTS `individual_final_report`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `individual_final_report`  AS  select `s`.`shipment_id` AS `shipment_id`,`s`.`shipment_code` AS `shipment_code`,`s`.`shipment_date` AS `shipment_date`,avg(ifnull(`spm`.`shipment_score`,0) + ifnull(`spm`.`documentation_score`,0)) AS `mean_shipment_score` from (`shipment` `s` join `shipment_participant_map` `spm` on(`s`.`shipment_id` = `spm`.`shipment_id`)) where `s`.`is_official` = 1 and `s`.`shipment_id` <= 33 group by `s`.`shipment_id` order by `s`.`shipment_date` desc limit 6 ;
...
ALTER TABLE `reference_result_tb`
  ...
  ADD KEY `is_excluded` (`is_excluded`),
  ADD KEY `shipment_id_2` (`shipment_id`,`sample_id`,`is_excluded`),
  ADD KEY `shipment_id_3` (`shipment_id`,`sample_id`,`is_excluded`),
  ADD KEY `shipment_id_4` (`shipment_id`,`sample_id`,`is_excluded`),
  ADD KEY `shipment_id_5` (`shipment_id`,`sample_id`,`is_excluded`);
...
ALTER TABLE `response_result_tb`
  ADD KEY `sample_id` (`sample_id`),
  ADD KEY `shipment_map_id` (`shipment_map_id`),
  ADD KEY `shipment_map_id_2` (`shipment_map_id`,`sample_id`),
  ADD KEY `shipment_map_id_3` (`shipment_map_id`,`sample_id`),
  ADD KEY `shipment_map_id_4` (`shipment_map_id`,`sample_id`);
...
ALTER TABLE `r_tb_assay`
  ...
  ADD KEY `id` (`id`),
  ADD KEY `id_2` (`id`),
  ADD KEY `id_3` (`id`,`short_name`),
  ADD KEY `id_4` (`id`,`short_name`),
  ADD KEY `id_5` (`id`,`short_name`),
  ADD KEY `id_6` (`id`,`short_name`),
  ADD KEY `id_7` (`id`,`short_name`),
  ADD KEY `id_8` (`id`,`short_name`);
...
ALTER TABLE `shipment`
  ...
  ADD KEY `shipment_id` (`shipment_id`),
  ADD KEY `shipment_id_2` (`shipment_id`),
  ADD KEY `shipment_id_3` (`shipment_id`);
