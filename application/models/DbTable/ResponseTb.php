...
class Application_Model_DbTable_ResponseTb extends Zend_Db_Table_Abstract {
    ...
    public function updateResult($params) {
        ...
        $dataManagerId = $authNameSpace->dm_id;
        $res = $this->fetchRow("shipment_map_id = " . $params['smid'] . " and sample_id = " . $sampleId);
        $instrumentInstalledOn = Pt_Commons_General::dateFormat($params['instrumentInstalledOn']);
        if (!isset($params['instrumentInstalledOn']) ||
            $params['instrumentInstalledOn'] == "") {
            $instrumentInstalledOn = null;
        }
        $instrumentLastCalibratedOn = Pt_Commons_General::dateFormat($params['instrumentLastCalibratedOn']);
        ...
        if ($res == null || count($res) == 0) {
            $this->insert(array(
                'shipment_map_id' => $params['smid'],
                'sample_id' => $sampleId,
                'date_tested' => Pt_Commons_General::dateFormat($params['dateTested']),
                ...
                'cartridge_expiration_date' => Pt_Commons_General::dateFormat($params['cartridgeExpirationDate']),
